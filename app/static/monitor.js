...
const state = {
  ...
  stress: null, sentiment: null, cryptoOverview: null, cryptoSentiment: null, cryptoVolatility: null, cryptoKimchi: null, cryptoStructure: null, cryptoGas: null, cryptoBoard: null, cryptoRegime: null, cryptoNews: null, bioTrials: null, bioFda: null, bioAdcomm: null, bioMfds: null, bioOutcomes: null, bioMfdsFilter: "notable", krOvernight: null, krPension: null, krHoldings: null, krEtf: null, krSearchInterest: null, ksiSort: "spike", usPtr: null, usOvernight: null, calendar: null,
...
const DISABLED_CODES = {
  ...
  bio_outcomes_disabled: "status.disabled",
  bio_outcomes_collecting: "status.collecting",
...
function renderJumpNav() {
  ...
  [{ id: "kr-overnight", text: t("kro.title") },
    ...
    { id: "bio-outcomes", text: t("bio.outcomes.title") },
...
function renderAttribution() {
  ...
  const laneReasons = [...new Set(["macro", "assets", "weekend", "sectors", "correlation", "stress", "sentiment", "cryptoOverview", "cryptoSentiment", "cryptoVolatility", "cryptoKimchi", "cryptoStructure", "cryptoGas", "cryptoBoard", "cryptoRegime", "cryptoNews", "bioTrials", "bioFda", "bioAdcomm", "bioMfds", "bioOutcomes"]
...
async function loadCore() {
  ...
  const [macro, assets, sectors, weekend, stress, sentiment, krIndices, krOvernight, krPension, krPensionPortfolio, krHoldings, krEvents, krEtf, usPtr, usManagers, usOvernight, usEvents, calendar, feed, cryptoOverview, cryptoSentiment, cryptoVolatility, cryptoKimchi, cryptoStructure, cryptoGas, cryptoBoard, cryptoRegime, cryptoLiquidations, cryptoNews, bioTrials, bioFda, bioAdcomm, bioMfds, bioOutcomes, newsVideos, krSearchInterest] = await Promise.all([
    ...
    request("/api/bio/outcomes", "bioOutcomes"),
...
function bioGateHidden(key) {
  ...
  return code === "bio_section_disabled" || code === "bio_trials_disabled" || code === "bio_fda_disabled" || code === "bio_adcomm_disabled" || code === "bio_mfds_disabled" || code === "bio_outcomes_disabled";
}
...
function renderBioOutcomes() {
  const section = $("#bio-outcomes"); if (!section) return;
  const stateNode = $("#bioo-state"), panel = $("#bioo-panel"), table = $("#bioo-table"), footer = $("#bioo-footer");
  const data = state.bioOutcomes;
  if (!data || !Array.isArray(data.events)) {
    if (bioGateHidden("bioOutcomes")) { section.hidden = true; return; }
    const collecting = disabledCode("bioOutcomes") === "bio_outcomes_collecting";
    section.hidden = false; panel.hidden = true; stateNode.hidden = false;
    stateNode.classList.toggle("disabled", collecting);
    stateNode.textContent = collecting ? t("status.collecting") : `${t("status.unavailable")} · ${t("status.retry")}`;
    return;
  ...
  const pct = (value, suffix) => (typeof value === "number" ? `${value > 0 ? "+" : ""}${value.toFixed(1)}${suffix}` : "—");
  const tone = (cell, value) => { if (typeof value === "number" && value !== 0) cell.classList.add(value > 0 ? "up" : "down"); return cell; };
  const cpCell = (row, horizon) => {
    const cp = (row.checkpoints || []).find((c) => c.horizon === horizon);
    if (!cp) return bioCell("—");
    if (cp.status === "halted") return bioCell(t("bio.outcomes.halted"));
    return tone(bioCell(pct(cp.excess, "%p")), cp.excess);
  };
  const tbody = bioHead(table, [t("bio.col.permitDate"), t("bio.col.item"), t("bio.col.company"), t("bio.col.base"), t("bio.col.live"), t("bio.col.m1"), t("bio.col.m3"), t("bio.col.m6")]);
  const rows = data.events;
  if (!rows.length) bioEmptyRow(tbody, 8, t("bio.outcomes.none"));
  rows.forEach((row) => {
    const tr = document.createElement("tr");
    const item = document.createElement("span"); item.className = "bio-title";
    if (row.url) { const link = document.createElement("a"); link.href = row.url; link.target = "_blank"; link.rel = "noopener noreferrer"; link.textContent = row.title || row.event_id || "—"; item.append(link); }
    else item.append(document.createTextNode(row.title || "—"));
    const flags = document.createElement("span"); flags.className = "bio-flags";
    if (row.newdrug_class) { const badge = document.createElement("span"); badge.className = "status-badge"; badge.textContent = `${t("bio.mfds.flag.newDrug")} · ${row.newdrug_class}`; flags.append(badge); }
    if (row.rare) { const badge = document.createElement("span"); badge.className = "status-badge"; badge.textContent = t("bio.mfds.flag.rare"); flags.append(badge); }
    if (flags.childNodes.length) item.append(flags);
    const companyCell = document.createElement("span");
    const stockLink = document.createElement("a"); stockLink.href = `/stock/${row.stock_code}`; stockLink.textContent = row.company || row.stock_code || "—";
    companyCell.append(stockLink);
    const base = row.base || {};
    let baseCell;
    if (base.status === "ok" && typeof base.close === "number") {
      baseCell = bioCell(`${Math.round(base.close).toLocaleString()}원`);
      if (base.halted) { const badge = document.createElement("span"); badge.className = "status-badge warn"; badge.textContent = t("bio.outcomes.halted"); baseCell.append(document.createTextNode(" "), badge); }
    } else baseCell = bioCell(base.status === "no_data" ? t("bio.outcomes.noData") : t("bio.outcomes.baseWait"));
    const live = row.live || {};
    const liveCell = tone(bioCell(pct(live.vs_base_percent, "%")), live.vs_base_percent);
    tr.append(
      bioCell(row.event_date || "—"),
      bioCell(item),
      bioCell(companyCell),
      baseCell,
      liveCell,
  ...
  link.textContent = state.lang === "en" ? (data.attribution?.text_en || "Source: MFDS / FSC (data.go.kr)") : (data.attribution?.text || "출처: 식품의약품안전처 · 금융위원회 (공공데이터포털)");
  attribution.append(link);
  if (data.as_of) attribution.append(document.createTextNode(` · ${t("date.asof")} ${dateText(data.as_of)}`));
  if (data.counts) attribution.append(document.createTextNode(` · ${t("bio.outcomes.matched")} ${data.counts.total}`));
  const method = document.createElement("p"); method.className = "kro-method"; method.textContent = localValue(data.methodology, state.lang);
  const disclaimer = document.createElement("p"); disclaimer.className = "kro-disclaimer"; disclaimer.textContent = localValue(data.disclaimer, state.lang);
  footer.append(attribution, method, disclaimer);
}
