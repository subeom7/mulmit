...
Only ``/v1/global-metrics/quotes/latest`` is used (one credit per call).  The
ingest lane stores a small blob every ``CMC_MAX_AGE`` seconds and the request
path reads that blob, so the monthly budget is a few thousand credits at most.
"""
