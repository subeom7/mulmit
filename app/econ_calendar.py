...
* **미국 데이터 발표 일정**은 FRED 릴리스 메타데이터(release/dates)에서
  ...
  수집은 ingest 배치에서만 돈다.
* **정책회의(FOMC·한국은행 금통위)**는 FRED에 쓸 만한 형태가 없어 공식
  페이지에서 확인한 날짜를 코드에 박는다. 확인일과 출처 URL을 데이터에
  동봉하고, "직전 회의 전까지 잠정"이라는 기관 문구 취지를 basis로 전달한다.
  분기·연 단위로 얼마 안 되는 날짜라 큐레이션이 정직한 최소 비용이다.
...
# (결과 발표일)이다. 새 일정이 공표되면 이 표와 확인일을 함께 갱신한다.
...
def build_calendar(today: dt.date | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "basis_ko": (
            ...
            "발표일은 FRED 릴리스 일정에서, FOMC·금통위 날짜는 각 기관 공식 "
            f"페이지에서 가져왔습니다(확인일 {CURATED_VERIFIED_AT}). FOMC 일정은 "
            "직전 회의 전까지 잠정입니다."
        ),
        "basis_en": (
            "Scheduled dates as announced by the institutions; they can change. US "
            "data-release dates come from FRED release metadata, and FOMC/BOK "
            f"meeting dates from the official calendars (verified {CURATED_VERIFIED_AT}). "
