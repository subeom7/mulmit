...
### 7.7 Phase 4b — 시장 국면 신호 (2026-08-22)

- 요청: "기술적 지표를 보고 신중해야 할 때인지 과감해야 할 때인지 신호". **매수·매도 화살표는 만들지 않는다** — 그건 투자 자문이고, 거짓 정밀도다.
  대신 **상태(regime)를 서술하는 두 숫자**를 코인 페이지에 둔다: `heat` 0~100(얼마나 쏠리고 늘어나 있나)과 `direction` −100~+100(추세가 어느 쪽인가).
  `#17` 시장 심리 게이지·스트레스 지수와 같은 계열이며, 같은 원칙(구성요소 원값 공개·합성식 공개·"신호 아님" 고지)을 따른다.
- 구성(`app/crypto_signal.py`, 가중치는 응답에 동봉): 펀딩 |APR| 30%(0%→0, 15%→50, 30%→80, 60%→100 구간 선형 — 15·30%는 카드 배지와 같은 기준),
  30일 실현변동성의 1년 백분위 25%, 90일 고저 범위 내 위치 25%, RSI(14)의 50 초과분 20%. 추세 = 20·50일 이동평균 구조 60% + RSI의 50 대비 위치 40%.
  밴드: 냉각(<25)·안정(<45)·보통(<65)·과열 주의(<80)·과열(≥80), 추세는 하락 우위(<−25)·방향성 없음·상승 우위(>25).
- 입력은 **전부 기존 릴레이 데이터**: HL 일봉(`candleSnapshot`, 최대 400일)과 코인 카드 컨텍스트. 새 공급자·새 권리 없음. 차트 인터벌을 바꿔도 신호는
  일봉 기준으로 고정된다(보는 화면에 따라 값이 달라지지 않게).
- 표시: 과열도·추세 게이지 + **한 문장 해석**(온도를 가장 많이 끌어올린 구성요소 2개를 이름으로 지목) + 구성요소 표(현재 값·기여도 막대·가중치) +
  방법론·면책. 면책 문구는 "과열이라고 곧 하락한다는 뜻이 아니다"까지 명시한다.
- 데이터가 60일 미만이면 `insufficient_data`로 섹션을 숨긴다(신상장 시장에서 의미 없는 숫자를 만들지 않는다).

### 7.8 Phase 4c — 시장 국면·카드 배지, 그리고 펀딩 기준선 정정 (2026-08-22)

- **실측에서 발견한 정정**: Hyperliquid 펀딩에는 8시간당 0.01%의 고정 이자 성분이 있어 **프리미엄이 0인 시장도 +10.95% APR**을 찍는다
  (2026-08-22 유동 시장 110개의 |APR| 최솟값 10.9%, BTC·ETH·ATOM·OP·ARB가 정확히 +10.95%). Phase 4b의 펀딩 과열도가 |APR| 자체를 쓰고 있어
  **중립 시장도 36점**을 받고 있었다 → **기준선과의 거리 |APR − 10.95|**로 정정. 정정 후 BTC 75→60, HYPE 80→50으로 내려가고 실제로 쏠린 ZEC(+259%)·
  XMR(+330%)만 100을 받는다.
- **시장 국면**(`/api/crypto/regime`, `app/crypto_regime.py`): 기준 코인(BTC) 과열도 35% + 펀딩 쏠림 폭 30% + 공포·탐욕 20% + 24h 상승 폭 15%.
  쏠림 폭 = 유동 시장(24h $1M↑) 중 펀딩이 기준선에서 15%p 이상 벌어진 비율(0→0, 15%→40, 35%→75, 60%→100). 추세 = BTC 추세 60% + 상승 폭 40%.
  구성요소가 빠지면(지수 수집 중 등) 남은 가중치로 재정규화한다. 표본이 HL 상장 퍼프임을 고지에 명시.
- **카드 배지**: 대시보드 코인 카드에 과열도 배지. 캔들 기반 절반은 ingest가 30분 주기로 저장하고(`crypto_coin_price_parts_v1`), **펀딩은 요청 시점에
  그 카드의 값으로 계산**해 합성한다 — 5초 폴링 경로에 업스트림 캔들 호출을 넣지 않으면서 배지와 카드의 펀딩이 어긋나지 않게.
- `crypto_signal`을 `price_components`(캔들) / `funding_component`(라이브) / `compose`로 분리해 코인 페이지·카드 배지·시장 국면이 같은 계산을 쓴다.

### 7.9 Phase 5a — 과열도·미결제약정 시계열 (2026-08-22)

- 국면 지표가 **점 값**이라 "어제보다 뜨거워졌나"를 알 수 없었다. 이미 30분마다 도는 코인 국면 lane에 **샘플 적재**를 얹었다(새 권리·새 공급자 없음,
  스냅샷 1회 추가 읽기).
- 블롭 `crypto_regime_history_v1`: 코인별·시장(`_market`) 시계열을 두 해상도로 — `recent`(최근 48샘플 ≈ 하루)와 `daily`(UTC 하루 1점, 90일).
  코인 샘플은 과열도·추세·가격·OI(USD)·펀딩 APR·거래대금, 시장 샘플은 쏠림 폭·상승 폭·전체 OI·유동 시장 수.
- **포지션 흐름**: 24시간 가격 변화와 OI 변화의 부호 조합을 표준 해석으로 이름 붙인다 — 가격↑·OI↑ 신규 롱 유입, 가격↑·OI↓ 숏 커버링, 가격↓·OI↑
  신규 숏 유입, 가격↓·OI↓ 롱 정리. 어느 한쪽이 ±0.5% 미만이면 "뚜렷한 변화 없음"으로 두고 이름을 붙이지 않는다.
- **정정(같은 날 라이브에서 발견)**: 샘플이 1개일 때 최신값을 자기 자신과 비교해 `변화 0.0 · 뚜렷한 변화 없음`을 내보내고 있었다 — 관측이 없는 것을
  '변화 없음'으로 지어내는 셈이라, 표본이 2개 미만이면 `changes.status = collecting`만 주고 파생 필드를 만들지 않는다.
- 표시: 코인 페이지 국면 패널에 24시간 과열도 변화·스파크라인·포지션 흐름, 대시보드 카드 배지에 `▲/▼` 변화량, 시장 국면 게이지에 쏠림 폭 추이
  스파크라인과 24h 변화. 샘플이 1개뿐이면 "수집 중"으로 표시하고 숫자를 만들지 않는다.

### 7.10 Phase 5b — 코인 페이지 원화 시세·김치프리미엄 (2026-08-22)

- 업비트 KRW 시장이 있는 코인(BTC·ETH·SOL·XRP·DOGE)의 코인 페이지에 **원화가·USDT 기준 프리미엄·공식환율 기준 프리미엄·USDT/KRW(테더 프리미엄)·
  업비트 24h 거래대금** 블록을 추가했다. 대시보드 김프 섹션과 **같은 빌더**(`crypto_kimchi.build_for_coin`)를 쓰므로 두 화면의 값이 어긋날 수 없다.
- 비용: Upbit 공급자에 단일 비행 TTL 캐시가 이미 있어 코인 페이지 요청이 추가 업스트림 호출을 만들지 않는다. 게이트(`UPBIT_ENABLED`)가 닫혀 있거나
...
### 7.12 Phase 5d — 차트 보조지표 (2026-08-22)

- 운영자 요구는 "업비트·코인베이스·트레이딩뷰처럼 차트 위에 이평선·보조지표·선그리기". TradingView **Advanced Charts**가 정답 경로지만
  신청·계약이 남아 있어(§7.6, 등록부 §3.2), 자체 SVG 차트에 **이평선·볼린저·RSI·MACD**를 먼저 붙였다. 선그리기는 Advanced Charts 몫으로 남긴다.
- **계산은 서버에서 한다**(`app/crypto_indicators.py`). 이유는 하나 — 코인 페이지는 이미 국면 패널에서 RSI를 말하고 있고,
  **같은 이름의 두 숫자가 다른 정의에서 나오면 안 된다**. `crypto_signal.rsi`(와일더)와 `crypto_indicators.rsi_series`의
  마지막 값이 같은지를 테스트가 직접 검증한다.
- 시리즈는 **캔들 배열과 길이가 같고**, 창이 안 찬 구간은 `None`이다. 프런트가 오프셋을 따로 들고 있지 않아도 같은 인덱스로 읽는다.
- 정의: SMA 20·50 / 볼린저 20±2σ(**모집단** 표준편차) / 와일더 RSI 14 / MACD 12·26·9(EMA는 첫 구간 단순평균으로 시드).
  **기간은 날짜가 아니라 봉 수**이므로 간격을 바꾸면 값이 바뀐다 — 이 문장을 `basis_ko`로 차트 아래에 함께 싣는다.
- 화면: 간격 버튼 아래 토글 줄(이평선·볼린저·RSI·MACD). RSI와 MACD는 **아래 페인 한 칸을 공유**해 서로를 끈다.
  볼린저는 밴드 사이를 옅게 채워 두 선이 아니라 한 봉투로 읽히게 했다. 밴드가 캔들 범위를 벗어나면 **가격 축을 넓힌다** —
  잘라내면 닿지 않은 밴드에 가격이 닿은 것처럼 보인다. 토글은 `localStorage`에 남고, 크로스헤어 툴팁이 그 봉의 지표값을 같이 읽어 준다.
- 권리: 새 원천 없음. 이미 게시 중인 캔들을 다시 말한 것이라 새 권리가 생기지 않는다.
...
| 날짜 | 내용 |
|---|---|
| 2026-08-22 | Phase 5d(§7.12) — 차트 보조지표(서버 계산 이평선·볼린저·RSI·MACD, 국면 패널 RSI와 정의 일치 테스트) |
...
| 2026-08-22 | Phase 4b(§7.7) — 시장 국면 신호(과열도·추세, 구성요소 4종 공개), 매수·매도 신호 아님 고지. 새 권리 없음 |
