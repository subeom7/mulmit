...
같은 lane이 H.10에서 **공식 환율**도 받는다 — 원·달러, 엔·달러, 위안·달러,
유로·달러, 파운드·달러. H.10은 호가 방향이 두 가지고 계열명이 유일한 단서라
(`$US`가 있으면 외화당 달러, 없으면 달러당 외화) 방향을 `units`에 문장으로 적어
저장한다. 반대로 읽으면 환율이 뒤집힌다. 이 값들은 HIP-3 합성 FX와 별도 key(`fx_*`)다.
...
`BLS_ENABLED`는 미 노동통계국에서 실업률을 받는다. 이 프로젝트에서 권리 근거가
가장 직접적으로 명시된 공급자다 — BLS가 자기 페이지에 "everything that we publish...
is in the public domain"이라고 쓰고, 조건은 출처 표기뿐이다. 키는 선택이며 없으면
하루 25회·10년, 있으면 500회·20년이다.

**Mulmit 유동성·스트레스 지수**(`GET /api/market/stress`)는 CNN Fear & Greed를
복제하지 않는다. 그 지수의 입력 대부분(변동성, 풋/콜, 하이일드)이 우리가 표시할
권리가 없는 것이라, 남은 것으로 만들면 심리 지수가 아니라 유동성·거시 스트레스
지수가 된다. 그래서 측정하는 것의 이름을 붙였다.

각 입력을 최근 5년 자기 이력 안의 백분위로 점수화하고, 스트레스가 큰 쪽이 높아지도록
방향을 맞춘 뒤 동일 가중으로 평균한다. 결측 입력은 채우지 않고 제외하며, 표시할
권리가 없는 계열은 입력에서 빠진다 — 합성 지수로 withheld 계열을 세탁해 내보내지
않기 위해서다. 입력이 3개 미만이면 지수를 아예 내지 않는다. 산식과 각 입력의 방향은
응답과 화면에 함께 실린다. 수익률 대비 백테스트는 하지 않았고, 예측이 아니다.

DOL 신규 실업수당은 **보류**했다. 기계판독 파일은 주별이고 컬럼이 `c1`~`c23`으로
익명이며, 전국 계절조정 헤드라인은 HTML 페이지와 PDF에만 있다. 컬럼을 추측하거나
페이지를 스크래핑하는 건 이 프로젝트가 금지한 방식이라 `initial_claims`는 빈
상태로 둔다.
...
`FSC_ENABLED`는 코스피·코스닥 지수와 삼성전자·SK하이닉스 종가를 **금융위원회
공공데이터**에서 받는다. 원 데이터는 한국거래소가 만들지만, 금융위원회가 이를
공공데이터로 개방하면서 data.go.kr에 이용허락범위 **"제한 없음"**, 비용 **무료**로
등록했다. 포털이 부여하는 가장 넓은 허락 등급이고, 이 lane이 기대는 근거는 그것
하나다.
...
대가는 신선도다. 기준일 **다음 영업일 13시 이후**에 공개되는 장 마감 확정값이라
금요일 종가는 월요일에 온다. 실시간 시세가 아니고, 이 lane 위에 실시간이라고 쓴
화면을 올리지 않는다.

공식 종가는 HIP-3 합성값과 **다른 카드**에 붙는다. `kospi_exact`·`kosdaq_exact`·
`samsung_exact`·`sk_hynix_exact`는 원화 장 마감값이고, `kospi`·`samsung`은 USD 환산
합성 무기한선물이다. 두 값을 한 시계열로 잇지 않는다. 그래서 `samsung` 카드의
alias에서 `005930`을 뺐다 — 그 코드는 이제 공식 종가를 가리키고, 양쪽이 같은
레코드를 집으면 서로 다른 측정값이 한 카드에서 섞인다.

```dotenv
FSC_ENABLED=false
FSC_API_KEY=            # data.go.kr 활용신청 후 발급. 인코딩·디코딩 키 모두 동작
```

같은 lane 위에 `/analytics`의 **국내 종목 검색·분석**이 있다. 전 종목 하루
스냅샷(약 2,900종목)을 로스터로 저장해 회사명·코드 검색은 로컬에서만 돌고,
종목을 고르면 공식 종가 5년치로 수익률·낙폭·MDD·변동성을 계산한다. 미수집
종목은 잠금 아래에서 한 번 즉시 수집한다 — 시간당 배치를 기다리게 하면 검색이
죽은 기능이 되기 때문이고, 이후 요청은 전부 DB 읽기다. 야후 시절 단일 종목
분석의 한국판이며, 미국 종목 분석은 라이선스된 가격 공급자가 생길 때까지
닫혀 있다.

키는 data.go.kr이 인코딩 키(`%2F`)와 디코딩 키(`/`) 두 형태로 보여준다. 이미
인코딩된 키를 한 번 더 인코딩하면 `%252F`가 되어 "등록되지 않은 서비스키"로
거절되는데, 오류 메시지가 인코딩을 가리키지 않아 원인을 찾기 어렵다. 어느 쪽을
넣어도 같은 형태로 정규화한다.

KRX OPEN API 어댑터와 테스트는 준비돼 있지만 공개 대시보드 이용·재배포 승인을
받지 않았으므로 기본값은 `KRX_ENABLED=false`다. 키 발급만으로 제3자 공개 권리가
생기는 것은 아니며, 정확한 이용 승인을 받기 전에는 배치에 연결하지 않는다. 위
`FSC_ENABLED`가 이걸 대신 열어 주지는 않는다 — 다른 데이터셋에 대한 별개의
허락이지, KRX 승인이 다른 경로로 도착한 게 아니다. 실시간 시세와 KRX 통계정보
전체는 여전히 KRX와의 별도 계약 영역이다.
