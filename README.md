...
| `/crypto` | 크립토 — 퍼프 시세, 김치프리미엄, 공포·탐욕, 도미넌스·스테이블코인, 펀딩·OI, 전체 시장 보드, 가스, 실현 변동성 |
| `/bio` | 바이오 — 임상 파이프라인(+PubMed 서지), FDA 자문위 공고, FDA 승인, 식약처 품목허가 |
| `/crypto/{심볼}` | 코인 상세 — Hyperliquid 캔들 차트와 시장 국면 신호(heat·direction). 서버가 심볼별 메타를 렌더한다 |
...
app/
  ...
  crypto_kimchi.py    업비트 원화 시세와 김치프리미엄
  ...
  crypto_regime.py    시장 전체 국면 + 코인별 캔들 파트 캐시(ingest)
...
정적 자산은 쿼리 버전(`?v=20260822-10`)으로 캐시를 깬다. `monitor.js`나
...
- 크립토 섹션의 공포·탐욕 지수는 [alternative.me](https://alternative.me/crypto/fear-and-greed-index/),
  ...
  표시한다. 외부 차트 위젯이나 제3자 가격 피드를 끼워 넣지 않는다.
