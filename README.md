# 📌 이 저장소는 스냅샷입니다 — 개발은 비공개로 계속됩니다

> [!IMPORTANT]
> **2026-08-27 동결.** [mulmit.com](https://mulmit.com)의 운영 개발은 비공개
> 저장소로 이동했고, 이 저장소는 첫 커밋부터 여기까지의 개발 기록 전체를
> 보존하는 **포트폴리오 스냅샷**입니다. 이후의 커밋·브랜치는 없습니다
> (uptime 워크플로의 외부 감시만 계속 돕니다).

**왜 비공개로 갔나** — 기능을 감추려는 것이 아니라 실무 제약 둘 때문입니다.

1. **TradingView Advanced Charts 계약** — 차트 라이브러리는 공개 저장소에 둘 수 없습니다.
2. **커뮤니티·채점 기능의 어뷰징 방어** — 랭킹·필터 로직이 공개되면 우회가 쉬워집니다.

**여기 남은 것** — 그대로 읽을 수 있는 기록입니다.

- 데이터 소스별 **권리 판정 기록**: 무엇을 합법적으로 표시할 수 있는지의 지도 (`docs/DATA_SOURCE_REGISTER.md`)
- 실측으로 판정한 로드맵과 사고·함정 기록 (`docs/ROADMAP.md`), 제품 방향 (`docs/DIRECTION.md`)
- 백엔드 전체와 1,300+ 테스트 — SSR·배치 수집·공시 채점 엔진의 실제 구현

라이브 서비스: **[mulmit.com](https://mulmit.com)**
